...
const EXPLICIT_OUTPUT_TYPE_MIGRATION = 'migration_explicit_output_types_v1';
const COMMUNITY_ARTIFACT_OUTPUT_MIGRATION = 'migration_community_artifact_output_v1';
const DEEPSEEK_FLASH_MODEL = 'deepseek-v4-flash';
const DEEPSEEK_OFFICIAL_BASE_URL = 'https://api.deepseek.com/v1';
const LEGACY_OUTPUT_TYPE_FIXES = new Map([
  ['write-product-launch', 'text'],
  ['write-long-context-brief', 'text'],
]);
const LEGACY_COMMUNITY_OUTPUT_TYPE_FIXES = new Map([
  [
    'c_写一个纯-css-的-loading-动画-主题_mrdpyj6b',
    {
      outputType: 'html',
      prompt: '写一个纯 CSS 的 loading 动画，主题是擂台对决，输出完整 HTML',
    },
  ],
]);
...
function lockDeepSeekServerTarget(input = {}) {
  const rawBaseUrl = String(input.baseUrl || input.base_url || '').trim();
  let url;
  try {
  ...
  if (url.hostname.toLowerCase() !== 'api.deepseek.com') return { ...input };
  if (url.protocol !== 'https:' || url.port || url.username || url.password) {
    const err = new Error('服务端 DeepSeek Key 只能请求 https://api.deepseek.com');
    err.status = 500;
    throw err;
  }
  return {
...
function ensureSchema(database) {
  database.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_published_runs_case_version
      ON published_runs(case_id, version DESC);

    CREATE TABLE IF NOT EXISTS featured_runs (
      case_id TEXT PRIMARY KEY REFERENCES cases(id) ON DELETE CASCADE,
      published_run_id TEXT NOT NULL UNIQUE REFERENCES published_runs(id) ON DELETE CASCADE,
      updated_at TEXT NOT NULL
    );
...
function migrateExplicitSeedOutputTypes(database) {
  const applied = database.prepare('SELECT 1 FROM meta WHERE key = ?').get(EXPLICIT_OUTPUT_TYPE_MIGRATION);
  if (applied) return;
  const seedCases = fs.existsSync(CASES_SEED)
    ? jsonParse(fs.readFileSync(CASES_SEED, 'utf8'), [])
    : [];
  const canonicalById = new Map(seedCases.map((item) => [item.id, item]));
  const update = database.prepare(
    `UPDATE cases
     SET output_type = ?, updated_at = ?
     WHERE id = ? AND source = 'official' AND output_type != ? AND prompt = ?`
  );
  const t = nowIso();
  runTx(database, () => {
    LEGACY_OUTPUT_TYPE_FIXES.forEach((outputType, caseId) => {
      const canonical = canonicalById.get(caseId);
      if (canonical?.prompt) update.run(outputType, t, caseId, outputType, canonical.prompt);
    });
    database
      .prepare('INSERT INTO meta (key, value) VALUES (?, ?)')
      .run(EXPLICIT_OUTPUT_TYPE_MIGRATION, t);
  });
}

function migrateCommunityArtifactOutputTypes(database) {
  const applied = database
    .prepare('SELECT 1 FROM meta WHERE key = ?')
    .get(COMMUNITY_ARTIFACT_OUTPUT_MIGRATION);
  if (applied) return;
  const update = database.prepare(
    `UPDATE cases
     SET output_type = ?, updated_at = ?
     WHERE id = ? AND source = 'community' AND output_type != ? AND prompt = ?`
  );
  const t = nowIso();
  runTx(database, () => {
    LEGACY_COMMUNITY_OUTPUT_TYPE_FIXES.forEach(({ outputType, prompt }, caseId) => {
      update.run(outputType, t, caseId, outputType, prompt);
    });
    database
      .prepare('INSERT INTO meta (key, value) VALUES (?, ?)')
      .run(COMMUNITY_ARTIFACT_OUTPUT_MIGRATION, t);
  });
}
...
function findEnrichCredentials() {
  ...
  const mapped = rows.map((r) => lockDeepSeekServerTarget({
...
function openDb() {
  ...
  seedAdmin(db);
  seedCases(db);
  migrateExplicitSeedOutputTypes(db);
  migrateCommunityArtifactOutputTypes(db);
...
function caseRowToPublic(row) {
  ...
  if (Object.hasOwn(row, 'published_run_id')) {
    const resultCount = Number(row.published_result_count) || 0;
    const successCount = Number(row.published_success_count) || 0;
    out.publishedRunSummary = row.published_run_id
      ? {
          version: row.published_version,
          publishedAt: row.published_at,
          successCount,
          failureCount: Math.max(0, resultCount - successCount),
          resultCount,
          note: row.published_note || '',
        }
      : null;
  }
  return out;
...
function inferCaseOutputType(row) {
  const explicit = row.outputType ?? row.output_type;
  if (typeof explicit === 'string' && explicit.trim()) {
    return explicit.toLowerCase() === 'html' ? 'html' : 'text';
  }
...
function listPublishedCases({ q = '', category = 'all', difficulty = 'all', ready = 'all' } = {}) {
  const database = openDb();
  let rows = database
    .prepare(
      `SELECT cases.*,
        featured_runs.published_run_id AS published_run_id,
        published_runs.version AS published_version,
        published_runs.published_at AS published_at,
        published_runs.note AS published_note,
        COALESCE(json_array_length(published_runs.results_json), 0) AS published_result_count,
        COALESCE((
          SELECT COUNT(*)
          FROM json_each(published_runs.results_json)
          WHERE json_extract(json_each.value, '$.status') = 'ok'
        ), 0) AS published_success_count
       FROM cases
       LEFT JOIN featured_runs ON featured_runs.case_id = cases.id
       LEFT JOIN published_runs ON published_runs.id = featured_runs.published_run_id
       WHERE cases.status = 'published'
         AND (? != 'only' OR featured_runs.published_run_id IS NOT NULL)
       ORDER BY cases.sort_order ASC, cases.updated_at DESC`
    )
    .all(ready);
...
function upsertModel(input) {
  input = lockDeepSeekServerTarget(input);
  const database = openDb();
  const t = nowIso();
...
function contributionRowToPublic(r) {
  if (!r) return null;
  return {
    id: r.id,
    caseId: r.case_id,
...
function getContribution(id) {
  const row = openDb().prepare('SELECT * FROM contributions WHERE id = ?').get(id);
  return contributionRowToPublic(row);
}
...
function listContributions(limit = 100) {
  return openDb()
    ...
    .all(limit)
    .map(contributionRowToPublic);
}

function deleteContribution(id) {
  const result = openDb().prepare('DELETE FROM contributions WHERE id = ?').run(id);
...
function runHistoryRowToPublic(r) {
  if (!r) return null;
  return {
    id: r.id,
    caseId: r.case_id,
...
function getRunHistory(id) {
  const row = openDb().prepare('SELECT * FROM run_history WHERE id = ?').get(id);
  return runHistoryRowToPublic(row);
}
...
function listRunHistory({ caseId = '', limit = 30 } = {}) {
  ...
  return rows.map(runHistoryRowToPublic);
}

function deleteRunHistory(id) {
  const result = openDb().prepare('DELETE FROM run_history WHERE id = ?').run(id);
  return Number(result.changes) > 0;
}

function publishedRunRowToPublic(row, { includeResults = true } = {}) {
  if (!row) return null;
  const results = jsonParse(row.results_json, []);
  const resultModels = results.map((result) => ({
    model: result.model || '',
  ...
  const out = {
    ...
    category: row.category,
    outputType: row.output_type === 'html' ? 'html' : 'text',
    note: row.note || '',
    resultCount: results.length,
    successfulResultCount: results.filter((result) => result.status !== 'error' && result.content).length,
    models: resultModels,
    isFeatured: !!row.is_featured,
    publishedAt: row.published_at,
  };
  if (includeResults) {
    out.system = row.system_prompt || '';
    out.prompt = row.prompt;
    out.rubric = jsonParse(row.rubric_json, []);
    out.results = results;
    out.runConfig = jsonParse(row.run_config_json, {});
  }
  return out;
}

function publishCaseRun({ caseId, results, runConfig = {}, note = '', systemPrompt = '' }) {
  const database = openDb();
  const caseRow = database.prepare('SELECT * FROM cases WHERE id = ?').get(caseId);
  if (!caseRow) return null;

  const id = `pub_${Date.now().toString(36)}_${randomToken(4)}`;
  const publishedAt = nowIso();
  let version;
  runTx(database, () => {
    version = database
      .prepare('SELECT COALESCE(MAX(version), 0) + 1 AS version FROM published_runs WHERE case_id = ?')
      .get(caseId).version;
    database
      .prepare(
        `INSERT INTO published_runs (
          id, case_id, version, case_title, category, system_prompt, prompt,
          rubric_json, output_type, results_json, run_config_json, note, published_at
        ) VALUES (
          @id, @case_id, @version, @case_title, @category, @system_prompt, @prompt,
          @rubric_json, @output_type, @results_json, @run_config_json, @note, @published_at
        )`
      ...
      .run({
        ...
        rubric_json: caseRow.rubric_json || '[]',
        output_type: inferCaseOutputType(caseRow),
    ...
    database
      .prepare(
        `INSERT INTO featured_runs (case_id, published_run_id, updated_at)
         VALUES (?, ?, ?)
         ON CONFLICT(case_id) DO UPDATE SET
           published_run_id=excluded.published_run_id,
           updated_at=excluded.updated_at`
      )
      .run(caseId, id, publishedAt);
  });
  return getPublishedRunByVersion(caseId, version);
}

function getFeaturedPublishedRun(caseId) {
  const row = openDb()
    .prepare(
      `SELECT published_runs.*, 1 AS is_featured
       FROM featured_runs
       JOIN published_runs ON published_runs.id = featured_runs.published_run_id
       WHERE featured_runs.case_id = ?`
    )
    .get(caseId);
  return publishedRunRowToPublic(row);
}

function listPublishedRuns({ caseId, limit = 20 } = {}) {
  const capped = Math.max(1, Math.min(100, Number(limit) || 20));
  const rows = openDb()
    .prepare(
      `SELECT published_runs.*,
        CASE WHEN featured_runs.published_run_id = published_runs.id THEN 1 ELSE 0 END AS is_featured
       FROM published_runs
       LEFT JOIN featured_runs ON featured_runs.case_id = published_runs.case_id
       WHERE published_runs.case_id = ?
       ORDER BY published_runs.version DESC
       LIMIT ?`
    )
    .all(caseId, capped);
  return rows.map((row) => publishedRunRowToPublic(row, { includeResults: false }));
}

function getPublishedRunByVersion(caseId, version) {
  const row = openDb()
    .prepare(
      `SELECT published_runs.*,
        CASE WHEN featured_runs.published_run_id = published_runs.id THEN 1 ELSE 0 END AS is_featured
       FROM published_runs
       LEFT JOIN featured_runs ON featured_runs.case_id = published_runs.case_id
       WHERE published_runs.case_id = ? AND published_runs.version = ?`
    )
    .get(caseId, version);
  return publishedRunRowToPublic(row);
}
...
function stats() {
  ...
  return {
    ...
    publishedRuns: database.prepare(`SELECT COUNT(*) AS c FROM published_runs`).get().c,
    featuredCases: database.prepare(`SELECT COUNT(*) AS c FROM featured_runs`).get().c,
  };
}
...
module.exports = {
  ...
  deleteRunHistory,
  publishCaseRun,
  getFeaturedPublishedRun,
  listPublishedRuns,
  getPublishedRunByVersion,
  verifyAdminPassword,
  setAdminPassword,
  createSession,
  destroySession,
